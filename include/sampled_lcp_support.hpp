...
private:
    ...
    inline std::size_t longest_common_phrase_prefix(dict_type& d, rmq_type& rmq, const sdsl::int_vector<>& lcp, std::size_t a, std::size_t b)
    {
        if(a == 0 || b == 0) { return 0; }
        ...
        std::size_t lcp_a_b_i = rmq(lcp_left, lcp_right);
        return lcp[lcp_a_b_i];
    }
...
public:
    ...
    recursive_slcp_support(const pfpds::dictionary<dict_l1_data_type>& l1_d, const pfpds::dictionary<parse_int_type, l2_colex_comp>& l2_d, const pfpds::parse& l2_p, pfpds::long_type is = 10)
...
private:
    ...
    void build()
    {
        ...
        for (std::size_t i = 1; i < l2_d.lcpD.size(); i++)
        {
            ...
            if(l2_d.d[l2_d.saD[i]] < int_shift or l2_d.d[l2_d.saD[i-1]] < int_shift){ continue; }
            for (std::size_t l = 0; l < l2_d.lcpD[i]; l++)
            {
                if(l2_d.d[l2_d.saD[i]+l] != Dollar)
                {
                    e_lcp_l2_D[i] += l1_d.length_of_phrase(l2_d.d[l2_d.saD[i]+l]-int_shift) - l1_d.w;
                }
                else
                {
                    e_lcp_l2_D[i] += 1;
            ...
            parse_int_type p_id_1 = l2_d.d[l2_d.saD[i - 1] + l2_d.lcpD[i]];
            parse_int_type p_id_2 = l2_d.d[l2_d.saD[i] + l2_d.lcpD[i]];
            ...
            e_lcp_l2_D[i] += longest_common_phrase_prefix(l1_d, l1_d.rmq_lcp_D, l1_d.lcpD, p_id_1 - int_shift, p_id_2 - int_shift);
        }
        ...
        for (std::size_t i = 0; i < l2_p.saP.size(); i++)
        {
            ...
            if (k > 0)
            {
                ...
                std::size_t lcp_p = longest_common_phrase_prefix(l2_d, rmq_e_lcp_l2_D, e_lcp_l2_D, l2_p.p[i + l], l2_p.p[j + l]);
